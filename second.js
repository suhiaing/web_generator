...
{
	name: "Complete Responsive Online Education Website Design Tutorial",
	sourceHtml: `
<!DOCTYPE html>
<html lang="en">
<head>
...
for (let i = 0; i < websites.length; i++) {
  ...
  // Give id for css
  newElement.id = `website-${i}`;

  // Give classList
  newElement.classList.add('newElementClass');

  // Set the data attribute to store the website ID
  newElement.dataset.id = i;

  // Set the innerHTML of the new element to the website title
  newElement.innerHTML = websites[i].name;

  // Append the new element to the target node
  targetNode.appendChild(newElement);

  // Append the website content to the new element
  newElement.appendChild(webContent);

  // Set the website content HTML
  webContent.innerHTML = websites[i].sourceHtml;

  // Hide the website content initially
  webContent.style.display = 'none';

  // Add an event listener to show the website content on click
  newElement.addEventListener('click', (event) => {
    const id = event.currentTarget.dataset.id;
    const webContent = document.getElementById(`website-${id}`).querySelector('div');

    // Toggle the display of the website content
    if (webContent.style.display === 'none') {
